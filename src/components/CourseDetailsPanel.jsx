...
const CourseDetailsPanel = ({ course, data, onClose }) => {
  ...
  const degrees = data.degrees
    .filter(
      (degree) => degree.course_array && degree.course_array.includes(course.id)
    )
    .map((degree) => degree.name);

  const majors = data.majors
    .filter(
      (major) => major.course_array && major.course_array.includes(course.id)
    )
  ...
  const renderPrerequisiteGroup = (group, depth = 0) => {
    ...
    const bgColor =
      group.type === "AND"
        ? `${blueShades[depth] || "bg-blue-500"} dark:bg-blue-900`
        : `${greenShades[depth] || "bg-green-500"} dark:bg-green-900`;
              ...
              <div>
                ...
                <div className="space-y-2">
                  {prerequisites.prerequisites.map((group, index) => (
                    <div key={index}>{renderPrerequisiteGroup(group)}</div>
                  ))}
                </div>
              </div>
            )}
